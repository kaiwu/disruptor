...
struct next_power_of_2 {
  constexpr static size_t value = power_of_2<X>::value > N
                                      ? power_of_2<X>::value
                                      : next_power_of_2<N, X + 1>::value;
};
...
struct padding_size {
  ...
  constexpr static size_t value =
      sizeof_is_more_than<T, N>::value
          ? next_power_of_2<sizeof(T)>::value - sizeof(T)
          : N - sizeof(T);
};

constexpr static size_t AUINT64_PADDING_SIZE =
    padding_size<std::atomic_uint_fast64_t, CACHE_LINE_SIZE>::value;
constexpr static size_t TIMEOUT_PADDING_SIZE =
    padding_size<struct timespec, CACHE_LINE_SIZE>::value;
...
struct ring_buffer {
  ...
  constexpr static size_t ENTRY_PADDING_SIZE =
      padding_size<T, CACHE_LINE_SIZE>::value;
  constexpr static size_t READER_PADDING_SIZE =
      padding_size<ring_buffer *, CACHE_LINE_SIZE>::value -
  ...
  struct alignas(CACHE_LINE_SIZE) reader {
    ...
    void block_read(F &&f, Args &&...args) {
      ...
      uint_fast64_t e = this->end.sequence;
      while (e >
             this->rb->tail_cursor.sequence.load(std::memory_order_relaxed)) {
      ...
      this->begin = e + 1;
      this->end.sequence.store(this->begin, std::memory_order_relaxed);
    }
  };
  static_assert(sizeof(reader) == 2 * CACHE_LINE_SIZE,
                "reader is exactly 2 CACHE_LINE_SIZE");
  ...
  static ring_buffer *create() {
    ...
    for (auto &r : rb->readers) {
      r.end.sequence = VACANT;
    }
    return rb;
  }

  entry *get(uint_fast64_t sequence) noexcept {
  ...
  reader *create_reader() {
    ...
    while (reader_num.count.load(std::memory_order_acquire) < R) {
      for (auto &r : this->readers) {
        if (r.end.sequence.compare_exchange_weak(vacant, 1,
  ...
  void spin_write(F &&f, Args &&...args) {
    ...
    while (true) {  // spin
      next_header =
          1 + this->head_cursor.sequence.load(std::memory_order_relaxed);
      last_reader = VACANT;
      for (auto &r : this->readers) {
        uint_fast64_t last = r.end.sequence.load(std::memory_order_relaxed) - 1;
        if (last_reader > last) {
          last_reader = last;
        }
      }
      ...
      if (LIKELY__(next_header - last_reader <= SIZE_MASK)) {
        ...
        if (this->head_cursor.sequence.compare_exchange_weak(
                desired, next_header, std::memory_order_relaxed,
                std::memory_order_relaxed)) {
          break;
        }
      }
    ...
    while (this->tail_cursor.sequence.load(std::memory_order_relaxed) !=
           next_header - 1) {
      // spin
    }
  ...
  void block_write(F &&f, Args &&...args) {
    uint_fast64_t last_reader;
    uint_fast64_t next_header =
        1 + this->head_cursor.sequence.fetch_add(1, std::memory_order_relaxed);
    while (true) {
      last_reader = VACANT;
      for (auto &r : this->readers) {
        uint_fast64_t last = r.end.sequence.load(std::memory_order_relaxed) - 1;
